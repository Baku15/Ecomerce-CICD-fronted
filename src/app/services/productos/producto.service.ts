...
export class ProductoService {
  ...
  getProductoById(productoId: string | number):Observable<any>{
    return this.http.get<[]>(`http://localhost:8080/ecomerce/producto/${productoId}`);
  }
...
get(id: number): Observable<Producto> {
  ...
  return this.http.put<any>(`http://localhost:8080/ecomerce/producto/${productoId}`, productoDto, { headers: headers }).pipe(
    catchError(error => {
